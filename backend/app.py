...
# Decoradores para verificar autenticação
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not current_user.is_admin:
            return jsonify({'erro': 'Acesso negado. Apenas administradores podem acessar esta funcionalidade.'}), 403
        return f(*args, **kwargs)
    return decorated_function
...
def atualizar_usuario(usuario_id):
    ...
    if usuario.id == current_user.id and 'ativo' in dados and not dados['ativo']:
...
def atualizar_perfil():
    ...
    if 'senha_atual' in dados and 'nova_senha' in dados:
        if not current_user.verificar_senha(dados['senha_atual']):
            return jsonify({'erro': 'Senha atual incorreta'}), 400
        current_user.definir_senha(dados['nova_senha'])
...
# Tratamento de erro para www
@app.before_request
def handle_www():
    # Remover www do domínio se presente
